...
在Java中，对基本数据类型的变量的读取和赋值操作是原子性操作，即这些操作是不可被中断的，要么执行，要么不执行。但是像`i++`、`i+=1`等操作字符就不是原子性的，它们是分成读取、计算、赋值几步操作，原值在这些步骤还没完成时就可能已经被赋值了，那么最后赋值写入的数据就是脏数据，无法保证原子性。被synchronized修饰的类或对象的所有操作都是原子的，因为在执行操作之前必须先获得类或对象的锁，直到执行完才能释放，这中间的过程无法被中断（除了已经废弃的`stop()`方法），即保证了原子性。
...
![](https://s2.ax1x.com/2019/07/15/ZodDhj.png)
...
![](https://s2.ax1x.com/2019/07/15/Zo68yj.png)

![](https://s2.ax1x.com/2019/07/15/Zo6Ymn.png)
...
* [详解Class类文件的结构（下）](https://mp.weixin.qq.com/s?__biz=MzU3OTkyMDAxNg==&mid=2247484033&idx=1&sn=9fde746119825b09faa08a36d142bec4&chksm=fd5f8b7eca2802682f03ce38f9d51fb59ec033c54cb776836161f7151939020bfe76113f3034&token=268872677&lang=zh_CN#rd)

## 3.2 同步代码块
我们新定义一个同步代码块，编译出class字节码，然后找到method方法所在的指令块，可以清楚的看到其实现上锁和释放锁的过程，截图如下：

![](https://s2.ax1x.com/2019/07/15/Zor8yR.png)

![](https://s2.ax1x.com/2019/07/15/Zortw6.png)
...
![](https://s2.ax1x.com/2019/07/15/ZoRMHs.jpg)
...
![](https://s2.ax1x.com/2019/07/15/ZoL1Lq.png)

## 5.3 锁粗化
**锁粗化是虚拟机对另一种极端情况的优化处理，通过扩大锁的范围，避免反复加锁和释放锁。**比如下面method3经过锁粗化优化之后就和method4执行效率一样了。

![](https://s2.ax1x.com/2019/07/15/ZoXmvj.png)
